	...
	// unverifyPIN()
       ...
       if (mScP15Card->card->reader->capabilities & SC_READER_CAP_PIN_PAD) {
               if (pinLength == 0) {
                       ...
                       // We are not block key entry from the keypad. As it is too late at
                       // this point; such should really be done by advertizing the right
                       // data in getAcl to the operating system.
                       //
